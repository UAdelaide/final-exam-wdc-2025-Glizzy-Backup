...
router.get('/walkrequests/open', async function (req, res, next) {
    try {
        ...
        res.json(requests.map(request_id: requests.request_id, dog_name: requests.dog_name, requested_time: requests.requested_time, duration_minutes: requests.duration_minutes, location: requests.loaction, owner_username: requests.username });
    } catch (e) {
