...
router.get('/walkrequests/open', async function (req, res, next) {
    try {
        ...
        res.json(requests.map(request => ({request_id: request.request_id, dog_name: request.dog_name, requested_time: request.requested_time, duration_minutes: request.duration_minutes, location: request.location, owner_username: request.username})));
    } catch (e) {
